...
app.get('/api/store-config', async (req, res) => { let conf = await prisma.storeConfig.findUnique({ where: { id: 1 } }); if (!conf) { conf = await prisma.storeConfig.create({ data: { id: 1 } }); } const admins = await prisma.user.findMany({ where: { role: { in: ['ADMIN', 'OWNER'] } }, select: { firstName: true, location: true, email: true, phone: true } }); res.json({ owner: conf, admins: admins }); });

app.post('/api/chat', async (req, res) => { 
    try { 
        const response = await fetch('https://api.deepseek.com/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.DEEPSEEK_API_KEY}` }, body: JSON.stringify({ model: 'deepseek-chat', messages: [{ role: 'system', content: `You are a helpful Support AI for AURA STORE.` }, { role: 'user', content: req.body.message }] }) }); 
...
app.post('/api/auth/send-profile-otp', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true });
    } catch(e) { console.error(e); res.json({ success: false, error: 'Failed to send OTP.'}); }
});
