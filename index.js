...
app.use(express.json({ limit: '50mb' })); // 🔥 INCREASED LIMIT FOR AVATAR UPLOAD
...
app.get('/api/verify-email/:token', async (req, res) => {
    ...
    if (!user) return res.status(400).send('<div style="background:#050b14; height:100vh; display:flex; justify-content:center; items-center;"><h2 style="color:#ef4444; font-family:sans-serif; margin-top:20%;">❌ Invalid or Expired Verification Link!</h2></div>');
    await prisma.user.update({ where: { id: user.id }, data: { isVerified: true, verifyToken: null } });
    res.send('<div style="background:#050b14; height:100vh; display:flex; justify-content:center; items-center;"><h2 style="color:#10b981; font-family:sans-serif; margin-top:20%;">✅ Email Verified Successfully! Redirecting to login...</h2><script>setTimeout(()=>window.location.href="/login", 2000);</script></div>');
});

// 🔥 FORGOT PASSWORD (CODE + LINK)
...
app.post('/api/reset-password', async (req, res) => {
    const { identifier, newPassword } = req.body; // identifier can be Token OR 6-Digit Code
