...
app.post('/api/promo', async (req, res) => {
  const promo = await prisma.promo.findUnique({ where: { code: req.body.code.toUpperCase() } });
  if(promo && promo.isActive) res.json({ success: true, discount: promo.discount });
  else res.json({ success: false, error: 'Invalid or expired Promo Code!' });
});

// BUY VIP
app.post('/api/buy-vip', async (req, res) => {
  const user = await prisma.user.findUnique({ where: { id: parseInt(req.body.userId) } });
  if(!user || user.balanceUsd < 30) return res.json({ success: false, error: 'Insufficient funds. VIP costs $30.' });
  const expiry = new Date(); expiry.setDate(expiry.getDate() + 30);
  await prisma.user.update({ where: { id: user.id }, data: { balanceUsd: { decrement: 30 }, isVip: true, vipExpiry: expiry } });
  res.json({ success: true, newBalance: user.balanceUsd - 30 });
});
...
app.post('/api/checkout', async (req, res) => {
  ...
  try {
    ...
    if(promoCode && !isVipActive) {
        ...
        if(p && p.isActive) discountMulti = 1 - (p.discount / 100);
    }
    ...
    for (let pId of uniqueCartItems) {
      ...
      if(isVipActive) priceNum = 0; else priceNum = priceNum * discountMulti;
...
// REGISTER V4 (WITH REFERRAL LOGIC)
app.post('/api/register', async (req, res) => { 
  try { 
    const { name, email, password, country, refCode } = req.body;
    let newRef = generateRefCode();
...
app.get('/admin', (req, res) => { const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress; if(ADMIN_ID) bot.telegram.sendMessage(ADMIN_ID, `🚨 *ACCESS DENIED*\nIP Logged: \`${ip}\``); res.status(403).send("ACCESS DENIED"); });
